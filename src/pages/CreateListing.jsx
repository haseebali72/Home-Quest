...
import { uploadImages } from '../firebase/fileUploads.firebase'
...
const CreateListing = () => {
    ...
    const formSubmission = async (data) => {
        setLoading(true)
        const response = await uploadImages(data.images)
        console.log(response)
        if (response.downloadURLs) {
            // console.log(response.downloadURLs)
            data.imageURLs = response.downloadURLs
            delete data.images
            console.log(data)
            const responseTwo = await addListing(data)
            console.log(responseTwo)
            if (responseTwo.docListed) {
                setLoading(false)
                toast.success(responseTwo.docListed)
            }

            if (responseTwo.errorinCatch) {
                setLoading(false)
                toast.error(responseTwo.errorinCatch)
            }
        } else {
            setLoading(false)
            toast.error("Error getting the image URLs")
            throw new Error("Error getting the image URLs")
        }
    }
    return (
        <>
